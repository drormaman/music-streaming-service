...
function Home() {
	...
	async function fetchSongs() {
		const data = await fetch("/api/top_songs");
		const songsArr = await data.json();
		setSongs(songsArr);
	}

	async function fetchArtists() {
		const data = await fetch("/api/top_artists");
		const artistsArr = await data.json();
		setArtists(artistsArr);
	}

	async function fetchAlbums() {
		const data = await fetch("/api/top_albums");
		const albumsArr = await data.json();
		setAlbums(albumsArr);
	}

	async function fetchPlaylists() {
		const data = await fetch("/api/top_playlists");
